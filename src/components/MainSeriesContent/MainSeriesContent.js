...
const MainSeriesContent = ({ searchValue }) => {
  ...
  const [areSearching, setAreSearching] = useState({ state: false, title: "Popular Series"})
      ...
      .then((data) => {
        ...
        fetch(API_URL + apiPageNumber)
            ...
            .then((data) => {
              setMoviesList(data.results)
              setResultsLength(true)
              setAreSearching({ state: false, title: "Popular Series"})
            })
      }
  }, [searchValue])
  ...
  return (
    <div id='content-container'>
      ...
      <SerieModal showModal={showModal} setShowModal={setShowModal} info={modalInfo} IMAGE_URL={IMAGE_URL}/>
...
export default MainSeriesContent;
